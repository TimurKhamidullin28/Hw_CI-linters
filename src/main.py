...
async def recipes() -> List[schemas.RecipeShortenedOut]:
    res: Any = await session.execute(select(
        models.Recipe.dish_name, models.Recipe.views, models.Recipe.cooking_time).order_by(
...
async def get_recipe(recipe_id: int) -> schemas.RecipeOut | tuple[str, int]:
    res: Any = await session.execute(select(models.Recipe).where(models.Recipe.id == recipe_id))
    recipe = res.scalar()
    if recipe:
        recipe.views += 1
